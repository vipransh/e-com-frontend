...
import { increamentQuantity, decrementQuantity, removeItem} from '../redux/cart';
import { validateCoupon } from './Helper/apiHelp';
...
function ProductCart() {
    ...
    const increase=(id)=>{
            dispatch(increamentQuantity({_id: id}))
    };
    const decrease=(id)=>{
    ...
    const couponClick=()=>{
     if(!coupon)
     {
      if(couponCode )
      {
        validateCoupon(couponCode).then(data=>{
          setCoupon(data?.coupon);
          if(data.coupon && data.coupon?.discount)
          {
            const dsc=Math.round((data.coupon && data.coupon?.discount*totalPrice)/100);
            setDiscount(dsc);
          }
        })
      }
    ...
    // send products, quantity, total price, coupon to checkout page
    const redirectToCheckoutPage=()=>{
      ...
      if(coupon)
      {
        navigate( '/checkout',
        {
          state: {products: cartList,quantity: quantity, totalPrice: totalPrice, coupon: coupon.code},}
        )
      }
      else
      {
    ...
    <div className=" md:w-2/3 w-full">
      ...
      <div className="flex flex-col md:ml-4 bg-white shadow-md rounded-xl md:w-1/3 w-full h-max py-6 px-4 ">
        ...
        <div>
          ...
          <div className="flex flex-row items-center justify-between border border-grey p-2 mt-4">
          <input onChange={(e)=>{
            setCouponCode(e.target.value)
          }} className='px-2' type="text" placeholder='Have Coupon..?'></input>
          <button onClick={couponClick} className='bg-blue-500 hover:bg-blue-700 text-white font-bold text-xs py-1 px-2 rounded'>APPLY</button>
          </div>
          <div className='flex items-start'>
          {coupon && coupon.discount>0 ? <h3 className='text-[#22CB5C] text-xs'>Coupon applied successfully..</h3>: ""}
          </div>
          ...
          <div onClick={redirectToCheckoutPage} className="flex justify-center  mt-4"><button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded">Check Out</button></div>
        </div>
      </div>
      </div>
    </div>
