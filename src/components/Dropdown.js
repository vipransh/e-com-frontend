...
function Dropdown() {
  ...
  return (
   <div>
    <BiUserCircle size={22} className="mr-4 cursor-pointer" onClick={dropDownHandle}/>
    {flag? <div className='bg-white text-black p-2 rounded-xl z-10 absolute top-11 right-8'>
      ...
      <Link to={"/"}><option className='hover:bg-gray-200' onClick={LogoutHandler}>Logout</option></Link>
    </div> : ""}
   </div>
  )
}
